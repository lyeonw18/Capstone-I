...
A reinforcement learning framework designed to jointly optimize predictive performance and fairness in high-stakes decision systems.
## 1. Overview
...
## 4.  Proposed Ethical RL Framework Architecture
...
### Evaluation Metrics
To ensure a rigorous comparison, we evaluated models using both performance and fairness metrics:
- **Accuracy**  
  Proportion of correctly predicted samples over the total number of samples.

- **Demographic Parity Difference (DPP)**  
  Measures the absolute difference in the probability of being predicted as “guilty” between demographic groups (e.g., age groups).  
  A lower value indicates smaller group-level prediction disparity.

- **Individual Fairness**  
  Measures the average prediction difference between similar individuals based on feature similarity.  
  A lower value indicates more consistent treatment of similar samples.
...
One notable limitation was the decile score variable:
 ...
 - showed weak performance in DPP experiments
However:
